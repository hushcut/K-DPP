...
기본 실행은 원본과 기본 전처리까지 이미지당 최대 2회의 외부 OCR만 사용합니다. 저장된
실사진 QA 캐시에는 반사 보정 후보의 정확도 기여 근거가 없어, 세 번째 반사 보정 OCR은
기본 비용에서 제외했습니다. 반사광 전용 실험에서는 다음 설정으로 후보를 활성화할 수
있습니다.
...
반사 보정을 활성화해도 원본·기본 전처리·반사 보정 후보의 **외부 OCR 대기 시간은 합계
최대 25초**까지만 사용합니다. 후보별 상한은 각각 10초·8초·7초이며, 남은 시간이 없으면
다음 후보를 생략하고 기존 결과와 `total_timeout` 진단 정보를 반환합니다. 이미지 검증과
전처리 시간은 이 외부 API 대기 예산과 별도입니다.
