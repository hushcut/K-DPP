...
/// 저장 시 `Clothes.materials`의 키로 옷장에 남는다. 반면 소재를 자동완성으로 고르면
/// `option.nameEn`이 들어가 영문 키가 된다. 즉 **같은 소재가 등록 경로에 따라 다른
/// 문자열로 저장된다.** 소재를 문자열로 판별하는 코드가 한쪽만 알아보면
/// 스캔한 옷에서만 조용히 기능이 죽는다.
///
