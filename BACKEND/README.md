...
옷장에 저장되는 건강도를 서버 왕복 없이 계산하므로, 이 표의 사본을 갖고 있습니다
(`FRONTEND/lib/utils/clothing_estimator.dart`). 사본이 시드와 어긋나면
`tests/test_material_name_contract.py`가 실패합니다 — `init_data.py`의 `MATERIAL_SEEDS`를
고칠 때는 그 파일도 함께 갱신해야 합니다.
